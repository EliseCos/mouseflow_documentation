...
Note that the below sections use ``nextflow`` version ``v23.04.2`` for
illustrative purposes: newer versions might work or be required depending on the
pipeline at issue.

⚠️ Mouseflow is only compatible with ``nextflow`` versions between 19.04.2 and 21.12.1 included. ⚠️
...
::
    ...
    $> wget https://github.com/nextflow-io/nextflow/releases/download/v23.04.2/nextflow && chmod +x nextflow && \
    echo 'export PATH=$PATH:'$(pwd) >> ~/.bash_profile && source ~/.bash_profile

High Performance computer (HPC)
###############################

1. Try ``module load nextflow/23.04.2`` or check with your administrator or on the HPC website.
2. Use ``wget`` to install Nextflow, change the name, add execution rights and add the Nextflow path in the bash_profile.

::

    $> wget https://github.com/nextflow-io/nextflow/releases/download/v23.04.2/nextflow-23.04.2-all && \
...
``module load nextflow/23.04.2`` (depending on the desired and available
...
If you want to use Docker on Windows or MacOS, please see the :ref:`docker-mouseflow` section.
